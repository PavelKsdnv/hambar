...
- Verify changes with **all five** headless smoke tests — `godot --headless --path . res://scenes/dev/<name>.tscn` for `CameraSmokeTest`, `WorldSmokeTest`, `BuildSmokeTest`, `SimSmokeTest` (the determinism harness: two runs from one seed, hashed every tick, reporting the first tick they differ on) and `CropSmokeTest`. Each prints PASS/FAIL and exits 0/1; the older ones are the regression net for the newer ones.
